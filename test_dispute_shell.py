...
def test_complete_dispute_workflow():
    ...
    try:
        ...
        if contract_manager.dispute_manager:
            print("✓ DisputeManager contract found")
            print(f"  Address: {contract_manager.dispute_manager.address}")
        else:
            print("✗ DisputeManager contract not found")
            return
        
        # Get a citizen user and a parcel that exists on blockchain
        citizen = User.objects.filter(role='citizen').first()
        parcel = Parcel.objects.get(id=70)  # Use the parcel we just registered
        
        if not citizen or not parcel:
            print("✗ No citizen user or active parcel found for testing")
            return
        
        print(f"✓ Using citizen: {citizen.username} and parcel: {parcel.id}")
        ...
        try:
            tx_hash = contract_manager.file_dispute(
                ...
                ipfs_hash="QmTestHash123456789"  # Use a test IPFS hash
            )
